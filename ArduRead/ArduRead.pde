...
void print_latlon2(AP_HAL::BetterStream *s, int32_t lat_or_lon)
{
...
void multiread(AP_HAL::RCInput* in, uint16_t* channels) {
    ...
    uint8_t valid;
    valid = in->readNoOverrides(channels, 8);
}

void multireadUSB(AP_HAL::RCInput* in, uint16_t* channels) {
    /* Multi-channel read method: */
    uint8_t valid;
    valid = in->readJustOverrides(channels, 8);
}
