...
static void NOINLINE send_baro_airspeed(mavlink_channel_t chan, float airspeed_val)
{
// HACK HACK HACK THIS IS A HUGE HACK SINCE I CANT FIGURE OUT WHY MY FUNCTION DOESNT SEND
// WHY DOESNT mavlink_msg_scaled_pressure_and_airspeed_send work???
mavlink_msg_scaled_pressure_send(
        chan,
        millis(),
        (float)airspeed_val,
        ...
        (int)(barometer.get_temperature()*10));
/*
    mavlink_msg_scaled_pressure_and_airspeed_send(
        chan,
        millis(),
        (float)barometer.get_pressure()/100.0f,
        (float)(barometer.get_pressure() - barometer.get_ground_pressure())/100.0f,
...
static void NOINLINE send_battery_status(mavlink_channel_t chan)
{
    uint16_t battery_current = -1;
    uint8_t battery_remaining = -1;
    ...
    mavlink_msg_battery_status_send(
        ...
        -1, // accupack ID
        battery_voltage1*1000.0f, // cell 1, in millivolts
        -1, // cell 2
        -1, // cell 3
        -1, // cell 4
        -1, // cell 5
        current_total1*100, // cell 6 -- WE USE CELL 6 AS THE TOTAL CURRENT DRAWN SINCE BOOT 10*milliamps (1 = 10 milliampere)
        battery_current, // battery current in 10*milliamps (1 = 10 milliampere)
        battery_remaining // Remaining battery energy: (0%: 0, 100%: 100)
        );
}
...
void
GCS_MAVLINK::send_message_imu_force()
{
    //send_attitude(chan);
    send_raw_imu1(chan);
}
...
void
GCS_MAVLINK::send_message_gps_force()
{
    send_gps_raw(chan);
}

void
GCS_MAVLINK::send_message_baro_airspeed_force(float airspeed_val)
{
    send_baro_airspeed(chan, airspeed_val);
}

void
GCS_MAVLINK::send_message_battery_status_force()
{
    send_battery_status(chan);
}
...
static void gcs_send_imu_force()
{
...
{
    gcs0.send_message_gps_force();
}

static void gcs_send_battery_status_force()
{
    gcs0.send_message_battery_status_force();
}

static void gcs_send_baro_airspeed_force(float airspeed_val)
{
    gcs0.send_message_baro_airspeed_force(airspeed_val);
}
